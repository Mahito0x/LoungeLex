...
// Check for Language (original, using cld)
...
function liofaMod(interaction, target) {
	...
	if (typeof UserRef === 'undefined') {
		...
		const timeSinceLastWarning = Date.now() - UserRef.time;
		
		// Reset warnings if it's been 24 hours since last warning
		if (timeSinceLastWarning >= TWENTY_FOUR_HOURS) {
			UserRef = { warnings : 1, time : Date.now() };
		}
		// If within warning cooldown window, increment warnings
		else if (timeSinceLastWarning < GuildData.Settings.time) {
			UserRef.warnings++;
			UserRef.time = Date.now();
		}
		// If outside cooldown but within 24 hours, start fresh warning
		else {
			UserRef.warnings = 1;
			UserRef.time = Date.now();
		}
